...
class MFLIpoll(Instrument):
    ...
    def _poll(self, module, number, setting):
        # rate = self.get("demod{}_samplerate".format(number))
        recording_time = 0.2
        timeout = 2
        node = "/{}/{}/{}/{}".format(self.device, module, number, setting)
        self.daq.subscribe(node)
        self.daq.sync()
        time.sleep(0.05)
        data = self.daq.poll(recording_time, timeout, 0, True)[node]
...
def main():
    test_instr = MFLIpoll("MFLI", "dev4264")
    print(test_instr._get_demod_sample(1, "x"))
    print(test_instr.parameters)
